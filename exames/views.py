...
def permitir_abrir_exame(request, exame_id):
    ...
    if exame.usuario != request.user:
        messages.add_message(request, constants.ERROR, 'Você não pode acessar esse exame')
...
def solicitar_senha_exame(request, exame_id):
    ...
    if exame.usuario != request.user:
        return messages.add_message(request, constants.ERROR, 'Você não está autorizado')
